...
## 3. Why This Space Resists Straightforward Approaches

### Why "build and measure" doesn't work here

Research users, build an MVP, measure, iterate. For ADHD workplace tools, this approach has specific structural problems:

- **Novelty Decay** inflates early engagement — users get genuine short-term benefit from new tools before the novelty effect expires (estimated 4–12 weeks in the literature)
- **System-Building as Substitute** — setup and customisation behaviour looks like engagement, but can be the user substituting the feeling of progress for the work itself
- **Circular Dependency** — users who complete onboarding are disproportionately unrepresentative of the target population, because the impairments that define severe ADHD are the ones that prevent consistent tool use

The conventional fallback — build it, measure retention at 30/60/90 days — is also compromised. The novelty cliff means 30-day retention tells you about novelty, not efficacy. And the population whose behaviour matters most is structurally the least likely to complete a sustained study, appear in feedback channels, or persist through onboarding.

User demand doesn't straightforwardly help either. The research consistently shows that ADHD users can articulate functional pain in vivid detail but struggle to describe what a solution would look like — wishlists tend to restate the problem rather than specify tool features. What demand signal does exist tends toward high-structure, high-frequency models (daily check-ins, planning rituals, habit trackers) — which is the archetype that existing ADHD productivity apps have overwhelmingly clustered around and failed in.

### Structural constraints on any ADHD product

Six meta-challenges apply to *any* ADHD workplace product, regardless of what problem it targets:

| Meta-challenge | Core issue |
|----------------|------------|
| **Novelty Decay** | New tools produce genuine short-term benefit that fades as novelty expires — early metrics are unreliable |
| **Circular Dependency** | The tool requires the executive function it's trying to support — users must remember to use the thing that helps them remember |
| **Shame-Avoidance Spiral** | Missed usage accumulates visibly, triggering shame, which causes avoidance, which causes more missed usage |
| **Disclosure / Workplace Visibility** | Using an ADHD-specific tool at work risks unwanted disclosure of a condition the user may not want visible |
| **Notification Desensitisation** | ADHD users develop notification blindness faster than neurotypical users — the primary delivery channel degrades |
| **System-Building as Substitute** | Customising and organising the tool itself becomes the activity, substituting for the work it was meant to support |

The 8 engagement models each have different exposure to these. The most revealing contrast:

- **Structured Check-In** (daily planners, habit trackers, check-in routines) has very high vulnerability to Novelty Decay, Circular Dependency, *and* Shame-Avoidance — all three at once. This is the dominant design pattern in existing ADHD apps. It is also what users most commonly describe wanting.
- **Passive Capture** (ambient monitoring, automatic logging, no user-initiated action required) has the lowest meta-challenge vulnerability — it sidesteps Circular Dependency entirely and has minimal Shame exposure. But it's the model with the least precedent and weakest evidence base.

The model users ask for is the most structurally vulnerable. The model with the best structural profile is the one nobody's asking for.

### The scale underneath

After filtering out problems that can't be addressed by software or by the individual user, the active space still contains 12 scored problems, 8 engagement models, 6 underlying mechanisms, and 3 overlapping clusters — before factoring in meta-challenge exposure, market positioning, and platform capabilities.

Any realistic product addresses multiple problems simultaneously. Problems overlap through clusters. Engagement models serve different problem subsets. This makes the solution space combinatorial, not linear — you're not picking one problem and one solution, you're choosing a *combination* across multiple dimensions.

*[TODO: calculate the actual combinatorial formula and insert the number here.]*

And the layers contradict each other. Two problems with the same severity score can have completely different mechanism profiles, which means they respond to different engagement models, which means they have different meta-challenge exposure. Problem scores point one way; meta-challenge vulnerability another; market gap a third. Single-dimension analysis is actively misleading, not just incomplete.
